...
describe('TESTING updateMultipleCustomFieldValues', () => {
	...
	describe('When the call is successful', () => {
		it('Shall return an object', async () => {
			...
			const result = await alation.updateMultipleCustomFieldValues(process.env.ALATION_DOMAIN, '', [
				{ field_id: 1, otype: 'test', oid: '1', value: ['test'] },
			]);
	...
	describe('When the call is unsuccessful', () => {
		it('Shall retry and throw an error', async () => {
			const expectedError = async () => {
				await alation.updateMultipleCustomFieldValues(process.env.ALATION_DOMAIN, '', [
					{ field_id: 1, otype: 'test', oid: '1', value: ['test'] },
				]);
			};
...
describe('TESTING getMultipleCustomFields()', () => {
	const url = encodeURI(
		`https://${
			process.env.ALATION_DOMAIN
		}/integration/v2/custom_field/?field_type=${'MULTI-PICKER'}&name_plural=${'Test'}`
	);
...
describe('TESTING getDatabases()', () => {
	const url = encodeURI(
		`https://${process.env.ALATION_DOMAIN}/integration/v1/datasource/?include_undeployed=false&include_hidden=true`
	);
...
describe('TESTING getColumn()', () => {
	const url = encodeURI(
		`https://${
			process.env.ALATION_DOMAIN
		}/integration/v2/column/?name=${'test'}&table_name=${'test'}&schema_id=${0}&ds_id=${0}`
	);
...
describe('TESTING getUsers', () => {
	const url = encodeURI(
		`https://${process.env.ALATION_DOMAIN}/integration/v1/user/?email=${'test@email.com'}&limit=100&skip=0`
	);
